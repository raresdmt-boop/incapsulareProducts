...
public class OrderDetails {
    ...
    public OrderDetails(String line) {
        ...
        setOrderID(Integer.parseInt(props[1]));
        setProductID(Integer.parseInt(props[2]));
        setPrice(Double.parseDouble(props[3]));
        setSku(Integer.parseInt(props[4]));
        setQuantity(Integer.parseInt(props[5]));
    }
    public OrderDetails(int orderID, int productID, double price, int sku, int quantity) {
        setOrderID(orderID);
        setProductID(productID);
        setPrice(price);
        setSku(sku);
    ...
    public void setId(int id) throws OrderDetailsIDCannotBeNegative {
        validateID(id);
        this.id = id;
    }
    ...
    public String toString() {
        return getId()+","+getOrderID()+","+getProductID()+","+getPrice()+","+getSku()+","+getQuantity();
    }
    @Override
    public boolean equals(Object obj) {
        return ((OrderDetails) obj).getId() == this.getId() && ((OrderDetails)obj).getOrderID() == this.getOrderID()
    ...
    private void validateID(int id) throws OrderDetailsIDCannotBeNegative {
        if (id < 0) {
            throw new OrderDetailsIDCannotBeNegative();
        }
    }
